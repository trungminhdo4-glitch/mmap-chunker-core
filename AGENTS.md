...
src/
  ...
  mmap.rs     — MmapFile: platform-specific mmap (Unix/Win), Send+Sync;
                 WindowedMmapFile: one bounded 64 KiB-aligned window at a time
...
| `mmap_engine_plan_partition_ranges(path,n,d,len,mode,win,out,cap,count)` | Source-selectable range planning, two-phase (v1.5) |
