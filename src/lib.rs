...
pub use ffi::{
    CChunkView, CEngineHandle, CFrameSpec, CPartitionRange, ABI_VERSION,
    CAP_CONFIGURABLE_DELIMITER, CAP_ERROR_STRINGS, CAP_FIXED_SIZE_CHUNKING, CAP_FRAMING_STRATEGIES,
    CAP_MULTI_BYTE_DELIMITER, CAP_MULTI_BYTE_PARTITIONING, CAP_RECORD_PARTITIONING,
    CAP_WINDOWED_PLANNING, CAP_ZERO_COPY, FRAME_KIND_DELIMITER, FRAME_KIND_FIXED_WIDTH,
    FRAME_KIND_LENGTH_PREFIXED, SOURCE_MODE_MMAP, SOURCE_MODE_PREAD, SOURCE_MODE_WINDOWED,
};
pub use framing::{
    BoundaryScanner, BuiltinFraming, FramingDescriptor, FramingStrategy, MAX_LENGTH_PREFIX_BYTES,
};
pub use index::{
    build_record_index, plan_partition_boundaries_from_index, RecordIndex, INDEX_SCHEMA,
    INDEX_SCHEMA_VERSION,
};
pub use manifest::{
    identify_file, plan_file, plan_file_with_framing, plan_from_ranges, FileIdentity,
    IndexReference, ManifestError, ManifestErrorKind, RangePlan, IDENTITY_SAMPLE_BYTES,
    MAX_MANIFEST_BYTES, MAX_MANIFEST_RANGES, PLAN_SCHEMA, PLAN_SCHEMA_VERSION,
};
pub use mmap::{MmapFile, WindowedMmapFile};
pub use scanner::ChunkCursor;
pub use scanner::PatternChunkCursor;
pub use source::{
    plan_partition_boundaries, plan_partition_boundaries_with, plan_partition_ranges,
    plan_partition_ranges_with, ByteSource, MmapSource, PlannerOptions, PreadSource, SourceMode,
    WindowedMmapSource, DEFAULT_SCAN_BUFFER_BYTES, DEFAULT_WINDOW_BYTES, MIN_WINDOW_BYTES,
};
