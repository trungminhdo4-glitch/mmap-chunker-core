...
//! buffered forward search whose overlap handling is the exact streaming
//! equivalent of [`scanner::find_partition_boundaries_pattern`].
...
pub(crate) fn find_pattern_from(
    ...
    loop {
        ...
        let read = source.read_at(offset, &mut buffer[..want])?;
        if read == 0 {
            return Ok(None);
        }
...
/// [`scanner::find_partition_boundaries_pattern`]. Sources that expose
/// a complete slice delegate directly to the slice scanner; all other
/// sources use a buffered forward search with exact parity.
///
...
pub fn plan_partition_boundaries(
    ...
    let mut buffer = vec![0u8; scan_buffer_bytes.max(delimiter.len()).max(1)];
    ...
    for i in 1..n {
        ...
        match find_pattern_from(source, target, delimiter, &mut buffer)? {
            Some(position) => {
                let boundary = position.saturating_add(delimiter.len()).min(file_len);
...
/// Compute N record-aligned partition boundaries using a pluggable
/// [`FramingStrategy`].
///
/// This is the format-agnostic core of the planner. Delimiter framing
/// through this function produces the same ranges as
/// [`plan_partition_boundaries`], which keeps an optimized slice fast
/// path; fixed-width and length-prefixed formats are planned with the
/// same target/assembly semantics.
///
/// The scan buffer is raised to the strategy's minimum requirement
/// (delimiter length or prefix width).
pub fn plan_partition_boundaries_with(
    source: &dyn ByteSource,
    num_partitions: usize,
    ...
    for i in 1..n {
        ...
        match boundary_scanner.boundary_after(target)? {
            Some(boundary) => {
                debug_assert!(boundary > target);
...
/// Plan record-aligned partition ranges for the file at `path` with a
/// pluggable [`FramingStrategy`] and selectable byte source.
///
/// Same backend selection and safety contract as
/// [`plan_partition_ranges`], but the record boundaries are defined by
/// the strategy (delimiter, fixed-width, length-prefixed, or a custom
/// implementation) instead of a raw delimiter.
///
/// # Safety
///
/// See [`plan_partition_ranges`].
pub unsafe fn plan_partition_ranges_with(
    path: impl AsRef<Path>,
    num_partitions: usize,
    strategy: &dyn FramingStrategy,
    options: &PlannerOptions,
    ...
    match options.mode {
        SourceMode::Mmap => {
            let source = unsafe { MmapSource::open_path(path)? };
            plan_partition_boundaries_with(&source, num_partitions, strategy, buffer)
        }
        SourceMode::Windowed => {
            let source = unsafe { WindowedMmapSource::open_path(path, options.window_bytes)? };
            plan_partition_boundaries_with(&source, num_partitions, strategy, buffer)
